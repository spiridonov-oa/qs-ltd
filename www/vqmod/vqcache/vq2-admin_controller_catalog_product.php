...
$this->data['entry_meta_title'] = $this->language->get('entry_meta_title');
		...
		$this->data['button_add_video'] = 'Add Video';
...
if (isset($this->error['meta_title'])) {
        ...
        // spiridonov.oa get product name for autocomplete new link
		$link_product_name = $this->data['product_description'][$this->config->get('config_language_id')]['name'];
		$symbol_position = strpos($link_product_name, "(");
        $this->data['link_name'] = substr($link_product_name, 0, $symbol_position);

		foreach ($product_options as $product_option) {
			if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image' || $product_option['type'] == 'link') {
				$product_option_value_data = array();
				
				foreach ($product_option['product_option_value'] as $product_option_value) {
				    if($product_option_value['link']) {
				        $link_product = $this->model_catalog_product->getProduct($product_option_value['link']);
				    } else {
				        $link_product = '';
				    }
					$product_option_value_data[] = array(
						...
						'link_name'               => $link_product ? $link_product['name'] : $this->data['link_name'],
