...
$product_title = "";
			...
			if ($product_info['quantity'] <= 0) {
				...
				$this->data['show_stock'] = true;
			} elseif ($this->config->get('config_stock_display')) {
				$this->data['stock'] = $product_info['quantity'];
				$this->data['show_stock'] = true;
			} else {
				$this->data['stock'] = $this->language->get('text_instock');
				$this->data['show_stock'] = false;
			}
            ...
            foreach ($results as $result) {
                $this->data['videos'][] = array(
                    'popup' => $this->model_tool_image->resize($result['image'], $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height')),
                    'thumb' => $this->model_tool_image->resize($result['image'], 300, 170),
                    'video'  => $result['video']
                );
            }
					...
					foreach ($option['option_value'] as $option_value) {
						if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
							...
							$option_value_data[] = array(
								...
								'link_id'                 => $option_value['link'],
								'link'                    => $option_value['link'] ? $this->url->link('product/product', 'product_id=' . $option_value['link']) : ''
							);
						}
					}
