...
$this->load->model('catalog/product');
                    $this->load->model('cart/personal_cart');
                    $resent_product_ids = array();
                    $resent_product_ids = $this->model_cart_personal_cart->getResentProductIds();
                    if(isset($this->session->data['cart'])) {
                        foreach ($this->session->data['cart'] as $key => $quantity) {
                            $product = explode(':', $key);
                            $resent_product_ids[] = $product[0];
                        }
                    }


                    $allData = array('limit' => 200, 'start' => 0);
                    $this->data['allProducts'] = array();
                    $allProducts = $this->model_catalog_product->getProducts($allData);
                    foreach ($allProducts as $key => $product) {
                        if(isset($resent_product_ids) && !in_array ($key, $resent_product_ids)) {
                    ...
                    if($this->customer->isLogged()) {
                        ...
                        $resent_products = $this->cart->getResentProducts($resent_product_ids);
                        foreach ($resent_products as $product) {
                            $product_total = 0;
                            ...
                            $this->data['resent_products'][] = array(
                                'key'                 => $product['key'],
                                'thumb'               => $image,
                                'name'                => $product['name'],
                                'model'               => $product['model'],
