...
class ControllerModulePersonalCart extends Controller {
	...
	public function index() {
    	...
    	if (true) {
            ...
            $this->data['products'] = $this->getCartProducts();


            /* +++ spiridonov.oa 31.01.2014 +++ */
            //Add recent products and all products to the data[]
            ...
            if($this->customer->isLogged()) {
                $this->data['recent_products'] = $this->getRecentProducts($recent_product_ids);
            }
            /* --- spiridonov.oa 31.01.2014 --- */
...
if($this->customer->isLogged()) {
                            ...
                            //Add products to db customer_product table
                            foreach ($this->session->data['cart'] as $key => $quantity) {
                                $product = explode(':', $key);
                                $product_id = $product[0];
                                $this->model_cart_personal_cart->addCustomerProduct($product_id);
                            }
                            /*
                            //Add resent products to the cart.
                            $resent_product_ids = $this->model_cart_personal_cart->getResentProductIds();
                            if ($resent_product_ids) {
                                $option = array();
                                $profile_id = '';
                                $qty = 0;
                                foreach($resent_product_ids as $product_id) {
                                    $this->cart->add($product_id, $qty, $option, $profile_id);
                                }
                            } */
                        }
				$products = $this->cart->getProducts();
				foreach($products as $key => $product){
				    $product_key = explode(':', $key);
                    $cart_product_id = $product_key[0];
				    if($product['product_id'] == $product_id){
				        $arr = array($product);
				        $product_data = $this->createProductsData(array($product));
                        $json['product'] = $product_data[0];
				    }
				}
...
if($this->customer->isLogged()) {
	...
	public function createProductsData($products) {
	    $products_data = array();
...
if($this->customer->isLogged()) {
        ...
        $cart_related_products = $this->model_cart_personal_cart->getCartRelatedProducts($cart_product_ids);

        foreach ($cart_related_products as $related_products) {
            // Add related products to data[] except current products in order
            foreach ($related_products as $key => $product) {
                if(isset($recent_product_ids) && !in_array ($key, $recent_product_ids)) {
